...
# Simple Index Creation Script for AcademicRAG RAG System
...
check_prerequisites() {
    ...
    if [ ! -f "run_system.py" ] || [ ! -d "rag_system" ]; then
        print_error "This script must be run from the AcademicRAG project root directory."
        exit 1
    fi
...
main() {
    ...
    print_status "You can now use the index in the AcademicRAG interface."
}
