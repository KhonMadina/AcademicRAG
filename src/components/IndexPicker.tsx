...
export default function IndexPicker({ onSelect, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white/15 backdrop-blur rounded-xl w-full max-w-xl max-h-full overflow-y-auto p-6 text-white space-y-6">
        <h2 className="text-lg font-semibold">Select an index</h2>
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search" className="w-full px-3 py-2 rounded bg-white/50 border border-white/50 text-black/80 hover:cursor-text" />
            ...
            {filtered.map(idx => (
              <li key={idx.id}>
                <div className="relative group">
                  <button onClick={()=>onSelect(idx.id)} className="w-full px-4 py-3 bg-white/10 hover:bg-white/20 rounded transition flex justify-between items-center pr-10 hover:cursor-pointer">
                    <span className="font-medium truncate max-w-[60%]">{idx.name}</span>
                    <span className="text-xs text-gray-100">{idx.documents?.length || 0} files</span>
                  </button>

                  <button onClick={(e)=>{e.stopPropagation(); setMenuOpenId(menuOpenId===idx.id?null:idx.id);}} title="More actions" className="absolute right-4 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 text-gray-100 hover:text-white transition text-lg leading-none font-bold">
