...
export function LandingMenu({ onSelect }: Props) {
  const Tile = ({ label, mode, icon }: { label: string; mode: Props["onSelect"] extends (m: infer U)=>void ? U: never; icon: React.ReactNode;}) => (
    <button
      ...
      className="w-56 h-44 rounded-xl bg-black/5 backdrop-blur border border-black/10 hover:border-black/30  flex flex-col items-center justify-center gap-2 transition  hover:cursor-pointer"
    >
      {icon}
