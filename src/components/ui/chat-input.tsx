...
export function ChatInput({ 
  onSendMessage, 
  disabled = false,
  placeholder = "Message Academic RAG...",
  className = "",
  onOpenSettings,
  onAddIndex,
  leftExtras
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (file.type === 'application/pdf' || 
        ...
        console.log('Frontend: Unsupported format; file refused:', file.type);
      }
    }
  ...
  return (
    <div className={`border-t border-white/10 rounded-2xl m-4 ${className}`}>
      <form onSubmit={handleSubmit} className="max-w-4xl mx-auto">
          ...
          <div className="mb-3 space-y-2">
            ...
            <div className="space-y-2">
              {attachedFiles.map((file) => (
                <div key={file.id} className="flex items-center gap-3 bg-gray-800 rounded p-3">
        ...
        <div className="bg-black/5 backdrop-blur border border-white/10 rounded-2xl px-5 pt-4 pb-3 space-y-2">
          ...
          <textarea
            ...
            rows={1}
            className="w-full bg-transparent border-none placeholder-gray-400 resize-none overflow-y-hidden focus:outline-none focus:ring-0 disabled:opacity-50 disabled:cursor-not-allowed text-base"
